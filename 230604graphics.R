...
#nxx <- 3:6
ndtcx <- x153$beta[,sort(unique(ndtc))] #already selected ndtc
x1 <- 
  copy(x153$beta)%>% #betas
  .[,lab:='']%>%
...
x2 <- #solution 4 15x DES/DRC p=const | indexed 1:20 on ndes
  x154[x153$beta,on=c(ndtc='ndtc',qq='qq')][,.(nx,ndtc,qq,b1,b2,b3,theta)] #20 dtiles, n=3:6; (ndtc-3)*5+qq = ndes{1:20}
...
x5 <-  #geo for des split; ndtc in 3:6
  x154[x153$beta,on=c(ndtc='ndtc',qq='qq')][,.(ndtc,qq,ndes)][geo4,on=c(ndes='ndes')]
...
x8 <- #x=theta_rep, y=lppm2, b2,b3
  ...
  x7[type=='des'][,.(type,ppm2,nx,qq)][x2[,.(nx,qq,ndtc,b1,b2,b3,theta)],on=c(nx='nx',qq='qq')][,.(type,ppm2,b1,b2,b3,theta,tbar=theta,qq),.(nx=ndtc)],
